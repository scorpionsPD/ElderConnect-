...
async function sendOTPEmail(email: string, otp: string): Promise<{ sent: boolean; error?: string }> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403 && isTestModeRestriction) {
        // In test mode - this is expected for unverified recipients
        // Log for dev visibility but don't fail the flow
        console.log('[INFO] Resend test mode: Email would be sent in production. Recipient must verify their email in Resend dashboard for testing.')
        
        // Return success for development continuity
        // In production, you'd verify your domain to send to all addresses
        return {
          sent: true  // Allow flow to continue
        }
      }
